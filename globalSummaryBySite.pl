...
sub printHeader {
  printf "Cluster\t\tTotal\tSuccess\tFailed\tSuccess Rate\tWritten($reportLabel)\n" if ($display==0);
  printf "<TABLE BORDER=1 ALIGN=center><TR BGCOLOR=lightgreen><TD ALIGN=center colspan='6'>$_[0] Report</TD></TR>" if ($display==1);
  printf "<TR BGCOLOR=lightgreen><TD>Cohesity Cluster</TD><TD>Total</TD><TD>Success</TD><TD>Failed</TD><TD>Success Rate</TD><TD>Written($reportLabel)</TD></TR>" if ($display==1);
}
...
sub dataGather {
  ...
  foreach my $href (@clusters){
    ...
    my $sql = "SELECT SUM(total_num_entities),SUM(success_num_entities),SUM(failure_num_entities),cluster_name,SUM(data_written_size_bytes) 
    ...
    #while(my @row=$sth->fetchrow_array){
      $sites{$rows[3]}="$rows[0],$rows[1],$rows[2],$rows[4]";
    #}
  }
}
...
sub printReport {
  ...
  foreach my $clusterName (sort keys %sites){
    ...
    my $dataWritten=$cols[3]/$reportValue;
    printf "$clusterName\t\t$cols[0]\t$cols[1]\t$cols[2]\t%2.1f%\t%2.1f\n",$successRate,$dataWritten if ($display==0);
    printf "<TR><TD>$clusterName</TD><TD ALIGN=center>$cols[0]</TD><TD ALIGN=center>$cols[1]</TD><TD ALIGN=center>$cols[2]</TD><TD ALIGN=right>%2.1f%</TD><TD>%2.1f</TD></TR>",$successRate,$dataWritten if ($display==1);
  }
