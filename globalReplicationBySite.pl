...
my $hoursAgo=24;
my %sites;
my $title="Global Cohesity Replication Report by Site";
...
sub printHeader {
  ...
  printf "<TABLE BORDER=1 ALIGN=center><TR BGCOLOR=lightgreen><TD ALIGN=center colspan='8'>All Sites Report</TD></TR>" if ($display==1);
...
sub gatherData{
  ...
  foreach my $href (@clusters){
    ...
    $sites{$cluster}="$totalJobs,$successJobs,$failedJobs,$activeJobs,$deltaSize,$logicalSize,$physicalSize";
    $dbh->disconnect();
  }
}
